...
def solve_maze(g, start, end):
    qu = []       # 앞으로 처리해야할 이동 경로 저장
    done = set()  # 이미 큐에 추가한 꼭짓점을 집합에 기록(중복 방지)

    qu.append(start)  # 출발점을 큐에 넣고 시작
    done.add(start)   # 집합에도 추가

    while qu:         # 큐에 처리할 경로가 남아 있는 동안
        p = qu.pop(0) # 큐에서 처리 대상을 꺼내
        v = p[-1]     # 큐에 저장된 이동 경로의 마지막 문자가 현재 처리해야할 꼭짓점
        if v == end:  # 처리해야할 꼭짓점이 도착점이면(목적지 도착)
    ...
    return '?'  # 탐색할 도착점이 나오지 않으면 나갈 수 없는 미로임
